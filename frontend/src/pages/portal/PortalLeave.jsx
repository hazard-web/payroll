...
export default function PortalLeave() {
  ...
  const fetchBalance = useCallback(async () => {
    try {
      // Try to get the current staff user's own profile which includes leaveBalance
      const res = await api.get('/portal/profile')
      if (res.data?.staff?.leaveBalance) {
        setBalance(res.data.staff.leaveBalance)
      }
    } catch {
      // If profile endpoint fails, leave balance at default
      console.warn('Could not fetch leave balance')
    } finally {
      setBalanceLoading(false)
    }
  }, [])

  useEffect(() => { fetchLeaveHistory(); fetchBalance() }, [fetchLeaveHistory, fetchBalance])
    ...
    return (
      <div className="pl-card">
        <div style={{ padding: '14px 20px', borderBottom: '1px solid var(--border)', display: 'flex', alignItems: 'center', gap: 8 }}>
          ...
          <span style={{ fontWeight: 700, fontSize: 14, color: 'var(--text)' }}>Leave Balance</span>
        </div>
        ...
        <div style={{
          ...
          CL &amp; SL are paid leaves. Excess leave beyond available balance will be marked as Leave Without Pay (LWP).
        </div>
      </div>
    )
  }

  function BalancePill({ label, value, subtitle, color, bg }) {
    return (
      <div style={{
        ...
        <div style={{ fontSize: 10, fontWeight: 700, color, textTransform: 'uppercase', letterSpacing: '.05em' }}>{label}</div>
        <div style={{ fontSize: 30, fontWeight: 800, color, lineHeight: 1.1, marginTop: 6 }}>
